...
export const getAllTicketsForTeam = createAsyncThunk('ticket/getAllTicketsForTeam', async (query, thunkAPI) => {
  try {
    return await TicketService.getAllTicketsForTeam(query);
  } catch (error) {
    return thunkAPI.rejectWithValue(getError(error));
  }
});
...
const ticketSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getAllTicketsForTeam.fulfilled, (state, action) => {
        console.log('getAllTicketsForTeam action payload', action.payload);
        state.isTicketLoading = false;
        state.isSuccess = true;
