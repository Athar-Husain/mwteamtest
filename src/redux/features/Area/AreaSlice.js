...
export const getMyAreasTeam = createAsyncThunk('serviceArea/getAll', async (_, thunkAPI) => {
  try {
    return await AreaService.getMyAreasTeam();
  } catch (error) {
    return thunkAPI.rejectWithValue(getError(error));
  }
});
...
const AreaSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // GET ALL
      .addCase(getMyAreasTeam.fulfilled, (state, action) => {
        state.isAreaLoading = false;
        state.isAreaSuccess = true;
