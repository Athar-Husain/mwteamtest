...
const ProfileIndex = () => {
  ...
  // Get Team data from Redux state
  const { Team, isTeamLoading } = useSelector((state) => state.team);
  ...
  //   console.log('Team in profile ', Team);
  ...
  useEffect(() => {
    if (Team) {
      reset({
        ...
        email: Team.email || '',
        phone: Team.phone || '', // assuming phone exists in model
  ...
  const onSubmit = (data) => {
    console.log('Update Team Action Triggered:', data);
  ...
  return (
    <>
      <Breadcrumbs links={[{ label: 'Dashboard', to: '/' }, { label: 'Team Profile' }]} divider />
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <Paper elevation={0} sx={{ p: 4, borderRadius: 4, textAlign: 'center', border: '1px solid #E0E7FF' }}>
                <Box sx={{ position: 'relative', display: 'inline-block' }}>
                  <Avatar
                    src={Team?.profileImage} // Map to your actual API key
                    sx={{
                      width: 120,
                      height: 120,
                      mb: 2,
                ...
                <Typography variant="body2" color="textSecondary" gutterBottom>
                  {Team?.role?.toUpperCase() || 'ADMINISTRATOR'}
                </Typography>
                ...
                <Box sx={{ textAlign: 'left' }}>
                  ...
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Typography variant="body2">ID: {Team?._id?.substring(0, 8)}...</Typography>
                  </Stack>
                </Box>
              </Paper>
            </Grid>
