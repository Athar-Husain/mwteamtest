...
import { getAllTicketsForTeam } from '../../redux/features/Tickets/TicketSlice';
...
const TicketsList = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        headerName: 'Status',
        width: 130,
        renderCell: (params) => <ModernChip label={params.value} variant="status" />
      },
      {
        field: 'priority',
        headerName: 'Priority',
        width: 120,
        renderCell: (params) => <ModernChip label={params.value} variant="priority" />
      },
      {
        field: 'timeInfo',
        headerName: 'Time',
        width: 180,
        renderCell: (params) => {
          const { status, createdAt, resolvedAt } = params.row;
          if (['resolved', 'closed'].includes(status)) {
            if (resolvedAt && createdAt) {
              const diff = dayjs(resolvedAt).from(dayjs(createdAt), true);
              return <Typography variant="body2">Resolved in {diff}</Typography>;
            }
            return <Typography variant="body2">—</Typography>;
          } else {
            const diff = dayjs().from(dayjs(createdAt), true);
            return <Typography variant="body2">Since {diff}</Typography>;
          }
        }
      },
      // {
      ...
      //     const name = agent ? `${agent.firstName} ${agent.lastName}` : 'You';
      //     const initials = agent ? `${agent.firstName?.[0] || ''}${agent.lastName?.[0] || ''}` : 'Y';
  ...
  return (
    <Box p={{ xs: 2, md: 4 }} sx={{ bgcolor: '#f8fafc', minHeight: '100vh' }}>
      ...
      <Paper sx={{ borderRadius: '16px', boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.1)', overflow: 'auto', border: '1px solid #e2e8f0' }}>
        <Box sx={{ p: 2.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: '#fff' }}>
          ...
          <Tooltip title="Refresh">
            <IconButton onClick={() => dispatch(getAllTicketsForTeam())}>
